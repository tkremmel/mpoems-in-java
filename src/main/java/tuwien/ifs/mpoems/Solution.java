...
public abstract class Solution implements Cloneable{
	...
	/**
	 * calculates the fitnessReward of this solution
	 * calculates the number of objectives of Solution s which are better or equal than the prototype solution objective
	 * fitnessReward value is used in the fitness assignment in order to reward
	 * solutions with a value between 0 and 1. the more objectives the solution dominates, the better the fitness value
	 * this is a improvement in comparison to the previously implemented fitness assignment,
	 * since fitness assignment is in this way more detailed.
	 * @param s
	 * @return
	 */
	/*public final float calculateFitnessReward(Solution s){
		double [] paramObjectives = s.getObjectives();
		float fitnessReward;
		int better_and_equal = 0;
		int worse = 0;
		int i = 0;
		while(i < this.nObjectives){
			switch(objectivesMaxMin[i]){
		...
		fitnessReward = (better_and_equal / new Float(this.nObjectives));
		//return fitnessReward;
		return fitnessReward;
	}*/
