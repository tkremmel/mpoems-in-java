...
public class ASPopulation extends ArrayList<ActionSequence> {
	 ...
	 * @return evaluated population of action sequences
	 */
	protected final void evaluate(Solution prototype, Solution prototypeClone, SolutionSet base) throws Exception{
		//time measuring is off
		//long startTime = System.currentTimeMillis();
		
		SolutionSet sSofOldPop = prototypeClone.applyActions(this);
		sSofOldPop.assignFront(base);//mention that prototype front value is as well reseted
		sSofOldPop.checkDomination(prototypeClone);
		Iterator<Solution> iter = sSofOldPop.iterator();
		int i = 0;
		...
		boolean new_fitness_calculation = false; //make it a config value
		
		if(!new_fitness_calculation){
			while (e < this.size()) {
		...
		/* modified
			1) that are in WORSE front than the prototype will have fitness
			    - a2.setFitness(base.size() + popSize – a2.getFront() - punishment), where punishment=0.5 if they are dominated by prototype and punishment=0 if they are not dominated by prototype.
			2) that are in BETTER front than the prototype will have fitness
			    - a2.setFitness(base.size() + popSize – a2.getFront() + reward), where reward=0.5 if they dominate the prototype and reward=0 if they do not dominate the prototype (without counting the number of solution’s objectives that are not worse).
		*/
		
		if(new_fitness_calculation){
			while (e < this.size()) {
				ActionSequence a2 = this.get(e);
				int prototypeFront = prototype.getFront();
				//System.out.println("prototype front is " + prototypeFront);
				if(a2.getFront() > prototypeFront){
					//actionsequence is on a worse front than the prototype 
					double punishment = 0;
					if(a2.getDominatedByPrototype() == 1){
						//System.out.println("is dominated by prototype");
						punishment = 0.5;
					...
					a2.setFitness(base.size() + popSize - a2.getFront() - punishment);
					//System.out.println("WORSE FRONT fitness is " + a2.getFitness());
				}
				if(a2.getFront() < prototypeFront){
					//actionsequence is on a better front than the prototype
					double reward = 0;
					if(a2.getDominatedByPrototype() == -1){
						//System.out.println("dominates the prototype");
						reward = 0.5;
						//System.out.println("reward is 0.5");
					} 
					else {
						//System.out.println("is not dominated");
						reward = 0;
					...
					a2.setFitness(base.size() + popSize - a2.getFront() + reward);
					//System.out.println("BETTER FRONT fitness is " + a2.getFitness());
				}
				if(a2.getFront() == prototypeFront){
					//actionsequence is on the same front like the prototype
					a2.setFitness(base.size() + popSize - a2.getFront());
					//System.out.println("SAME FRONT fitness is " + a2.getFitness());
				}
				this.set(e, a2);
				e++;
			}
